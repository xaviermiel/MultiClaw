 ...
 *
 * Compares oracle and chainlink-runtime-environment implementations
...
function compareStates(
  state1: SubAccountState,
  state2: SubAccountState,
  testName: string,
): TestResult {
  ...
  const allTokens = new Set([
    ...state1.acquiredBalances.keys(),
    ...state2.acquiredBalances.keys(),
  ]);
  ...
  return {
    ...
    details:
      differences.length === 0 ? "All checks passed" : differences.join("\n"),
  };
}
...
function runFuzzTest(numEvents: number, seed: number): TestResult {
  ...
  try {
    ...
    const protocolEventsCopy = JSON.parse(
      JSON.stringify(protocolEvents, (_, v) =>
        typeof v === "bigint" ? v.toString() : v,
      ),
    );
    const transferEventsCopy = JSON.parse(
      JSON.stringify(transferEvents, (_, v) =>
        typeof v === "bigint" ? v.toString() : v,
      ),
    );
    ...
    return compareStates(
      state1,
      state2,
      `Fuzz test (seed=${seed}, events=${numEvents})`,
    );
  } finally {
...
function testMultiTokenDeposit(): TestResult {
  ...
  const token1Balance = state.acquiredBalances.get(
    token1.toLowerCase() as Address,
  );
  if (token1Balance !== 250000000000000000n) {
    issues.push(
      `token1 balance: expected 250000000000000000, got ${token1Balance}`,
    );
  }

  // token2 should have 250000000000000000n remaining
  const token2Balance = state.acquiredBalances.get(
    token2.toLowerCase() as Address,
  );
  if (token2Balance !== 250000000000000000n) {
    issues.push(
      `token2 balance: expected 250000000000000000, got ${token2Balance}`,
    );
  }

  // lpToken should have 500000000000000000n (inherited from consumed tokens)
  const lpBalance = state.acquiredBalances.get(
    lpToken.toLowerCase() as Address,
  );
  if (lpBalance !== 500000000000000000n) {
    issues.push(
      `lpToken balance: expected 500000000000000000, got ${lpBalance}`,
    );
  }
  ...
  if (state.depositRecords.length !== 2) {
    issues.push(
      `depositRecords.length: expected 2, got ${state.depositRecords.length}`,
    );
  }
...
function testMultiTokenWithdraw(): TestResult {
  ...
  if (token1Balance !== 500000000000000000n) {
    issues.push(
      `token1 balance after withdraw: expected 500000000000000000, got ${token1Balance}`,
    );
  }
  ...
  if (token2Balance !== 500000000000000000n) {
    issues.push(
      `token2 balance after withdraw: expected 500000000000000000, got ${token2Balance}`,
    );
  }
...
function testMixedAcquiredNonAcquired(): TestResult {
  ...
  const tokenABalance = state.acquiredBalances.get(
    tokenA.toLowerCase() as Address,
  );
  if (tokenABalance !== undefined && tokenABalance !== 0n) {
    issues.push(
      `tokenA balance: expected 0 or undefined, got ${tokenABalance}`,
    );
  }
  ...
  const tokenBBalance = state.acquiredBalances.get(
    tokenB.toLowerCase() as Address,
  );
  if (tokenBBalance !== 800000000000000000n) {
    issues.push(
      `tokenB balance: expected 800000000000000000, got ${tokenBBalance}`,
    );
  }
...
function testExpiryWindow(): TestResult {
  ...
  const tokenBalance = state.acquiredBalances.get(
    token.toLowerCase() as Address,
  );
  if (tokenBalance !== 300000000000000000n) {
    issues.push(
      `token balance: expected 300000000000000000 (expired old), got ${tokenBalance}`,
    );
  }
  ...
  if (state.totalSpendingInWindow !== 50000000000000000n) {
    issues.push(
      `totalSpendingInWindow: expected 50000000000000000, got ${state.totalSpendingInWindow}`,
    );
  }
...
function testDuplicateTokens(): TestResult {
  ...
  try {
    ...
    const expected =
      500000000000000000n + 200000000000000000n + 200000000000000000n; // remaining + outputs
...
function testTransferConsumesAcquired(): TestResult {
  ...
  if (state.totalSpendingInWindow !== 150000000000000000n) {
    issues.push(
      `Expected spending 150000000000000000, got ${state.totalSpendingInWindow}`,
    );
  }
...
function testClaimWithoutDeposit(): TestResult {
  ...
  const balance = state.acquiredBalances.get(
    rewardToken.toLowerCase() as Address,
  );
  if (balance !== undefined && balance !== 0n) {
    issues.push(
      `CLAIM without deposit should not create acquired balance, got ${balance}`,
    );
  }
